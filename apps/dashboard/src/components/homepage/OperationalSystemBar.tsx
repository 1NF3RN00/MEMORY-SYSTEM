...
export function OperationalSystemBar({ indicators }: OperationalSystemBarProps) {
  const { openCommandPalette } = useShell();
  const [healthLabel, setHealthLabel] = useState(indicators.systemHealth);

  useEffect(() => {
    apiGet<{ status: string }>("/health")
      .then((res) => setHealthLabel(res.status === "ok" ? "nominal" : "degraded"))
      .catch(() => setHealthLabel("degraded"));
  }, []);
