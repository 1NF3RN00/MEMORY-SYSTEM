...
| Vercel build `P1001` during `db:migrate:deploy` | Do **not** run migrations in the Vercel build step — the build network often cannot reach Supabase `:5432`. Run `npm run db:migrate:deploy` locally or in CI before/after deploy |
